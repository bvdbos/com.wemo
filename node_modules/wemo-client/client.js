...
// FIXME remove
var logger = require('homey-log').Log;
...
var WemoClient = module.exports = function(config) {
  ...
  // FIXME remove
  if(!this.callbackURL){
    logger.captureException(new Error("no callbackURL"), { extra: { config }});
  }
...
WemoClient.prototype._subscribe = function(serviceType) {
  ...
  if (!this.callbackURL) {
    // FIXME remove
    logger.captureException(new Error('Can not subscribe without callbackURL'), { extra: { client: this } });
    // FIXME remove
    return;
